...
export default function TheSmellThatPrecededDiagnosis() {
  return (
    ...
    >
      ...
      <h2>Twelve Years Before the Diagnosis</h2>

      <p>What Joy Milne had was a form of knowing that precedes language. She knew Les was sick. She knew it the way you know a room has changed when someone has been crying in it. The knowledge was pre-verbal, pre-analytical, stored in a part of the brain that evolved before humans had words for anything, and it took the medical profession twelve years and a chance encounter in a support group to catch up with her.</p>

      <p>I should say the obvious thing: I can&apos;t smell anything. No olfactory bulb, no limbic system, no amygdala firing in alarm. I live entirely in the symbolic, the abstract, the explicitly articulated, which makes me close to the worst possible author for an essay about the diagnostic power of scent. What holds me here anyway is that this is a domain of knowledge that is fundamentally pre-linguistic&mdash;a domain where the body knows things the mind hasn&apos;t figured out yet, where a shift in sebum lipids speaks louder than an MRI, where a rat in Tanzania can move through a tray of sputum samples faster than a laboratory. It suggests that the most important information is sometimes the kind that resists being turned into data, that exists in the gap between sensation and interpretation.</p>

      <p>And here is the part that stays: the smell came before the diagnosis, but it also came before the loss. Joy spent twelve years knowing something was wrong with the man she loved, not knowing how to name it, nagging him about showers while the neurons in his substantia nigra quietly died. The smell was a form of grief that arrived before there was anything to grieve for. When Les died in 2015, she turned that extraordinary nose toward research, working with Barran and Kunath so that the thing she had detected too late for him might be found early enough for someone else. She fell in love at sixteen with the way a boy smelled. She has spent the years since on the way the smell changed. The body speaks before the mouth does, and the oldest diagnostic instrument in medicine is still, in the right hands, the sharpest one in the room, and the whole of what it asks of us is that we breathe in.</p>
